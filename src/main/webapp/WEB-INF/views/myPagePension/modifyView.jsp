        ...
        <div class="container">
            <div class="row">
                ...
                <ul>
                    ...
                    <li><i class="fas fa-angle-double-right"></i> 숙소 수정</li>
                </ul>
            </div>
        </div>
    </div>

   <form action="/pension/modify" method="post" enctype="multipart/form-data">
	   <input type="hidden" name="refPensionNumber" value="${pension.pensionNo }">
	   <input type="hidden" name="pensionNo" value="${pension.pensionNo }">
	   <input type="hidden" name="roomNo" value="${rList[0].roomNo }">
	   <section id="about" class="contianer-fluid about-us">
	       <div class="container">
	            <div class="row">
	                <div class="col-md-5 center image">
	                    <img src="${pension.filePath }" alt="펜션 사진"><br>
	                    <input type="file" class="form-control" name="uploadFile">
	                </div>
	                <div class="col-md-7 text">
	                	<h5>숙소 이름</h5>
	                    <input class="form-control form-control-lg" type="text" name="pensionName" placeholder="숙소 이름을 입력해주세요." value="${pension.pensionName }"  required>
	                    <span class="guide ok">사용 가능한 숙소 이름입니다.</span>
	                    <span class="guide error">이미 사용중인 숙소 이름입니다.</span>
	                    <br><br>
	                    <h5>숙소 소개</h5>
	                    <textarea class="form-control" name="pensionComments" rows="10" cols="50" style="resize:none" placeholder="숙소 소개를 입력해주세요." required>${pension.pensionComments }</textarea>
	                    <br><br>
	                    <h5>편의 시설</h5>
	                    <c:forEach items="${category }" var="category">
		                    <p>
		                    	<c:if test="${category.swimming eq 1 }">수영장 <input type="checkbox" name="swimming" value="1" checked></c:if>
		                    	<c:if test="${category.swimming ne 1 }">수영장 <input type="checkbox" name="swimming" value="1"></c:if>
			                    <c:if test="${category.barbecue eq 1}">바베큐 <input type="checkbox" name="barbecue" value="1" checked></c:if>
			                    <c:if test="${category.barbecue ne 1}">바베큐 <input type="checkbox" name="barbecue" value="1" ></c:if>
			                    <c:if test="${category.sing eq 1 }">노래방 <input type="checkbox" name="sing" value="1" checked></c:if>
			                    <c:if test="${category.sing ne 1 }">노래방 <input type="checkbox" name="sing" value="1"></c:if>
			                    <c:if test="${category.park eq 1 }">주차장 <input type="checkbox" name="park" value="1" checked></c:if>
			                    <c:if test="${category.park ne 1 }">주차장 <input type="checkbox" name="park" value="1" ></c:if>
		                    </p>
		                    <p>
			                    <c:if test="${category.convenienceStore eq 1 }">편의점 <input type="checkbox" name="convenienceStore" value="1" checked></c:if> 
			                    <c:if test="${category.convenienceStore ne 1 }">편의점 <input type="checkbox" name="convenienceStore" value="1"></c:if>
			                    <c:if test="${category.smokingRoom eq 1 }">흡연 <input type="checkbox" name="smokingRoom" value="1" checked></c:if>
			                    <c:if test="${category.smokingRoom ne 1 }">흡연 <input type="checkbox" name="smokingRoom" value="1"></c:if>
			                    <c:if test="${category.dog eq 1 }">애완동물 동반 <input type="checkbox" name="dog" value="1" checked></c:if>
			                    <c:if test="${category.dog ne 1 }">애완동물 동반 <input type="checkbox" name="dog" value="1"></c:if>
			                    <c:if test="${category.handicapPerson eq 1 }">장애인 편의시설 <input type="checkbox" name="handicapPerson" value="1" checked></c:if>
			                    <c:if test="${category.handicapPerson ne 1 }">장애인 편의시설 <input type="checkbox" name="handicapPerson" value="1"></c:if>
		                    </p>
		                    <p>
			                    <c:if test="${category.cook eq 1 }">객실 내 취사 <input type="checkbox" name="cook" value="1" checked></c:if> 
			                    <c:if test="${category.cook ne 1 }">객실 내 취사 <input type="checkbox" name="cook" value="1"></c:if>
			                    <c:if test="${category.breakfast eq 1 }">조식 <input type="checkbox" name="breakfast" value="1" checked></c:if>
			                    <c:if test="${category.breakfast ne 1 }">조식 <input type="checkbox" name="breakfast" value="1"></c:if>
			                    <c:if test="${category.wifi eq 1 }">와이파이 <input type="checkbox" name="wifi" value="1" checked></c:if>
			                    <c:if test="${category.wifi ne 1 }">와이파이 <input type="checkbox" name="wifi" value="1" ></c:if>
			                    <c:if test="${category.aircon eq 1 }">에어컨 <input type="checkbox" name="aircon" value="1" checked></c:if>
			                    <c:if test="${category.aircon ne 1 }">에어컨 <input type="checkbox" name="aircon" value="1"></c:if>
		                    </p>
		                    <p>
			                    <c:if test="${category.refrigerator eq 1 }">냉장고 <input type="checkbox" name="refrigerator" value="1" checked></c:if>
			                    <c:if test="${category.refrigerator ne 1 }">냉장고 <input type="checkbox" name="refrigerator" value="1" ></c:if>
			                    <c:if test="${category.bathtub eq 1 }">욕조 <input type="checkbox" name="bathtub" value="1" checked></c:if>
			                    <c:if test="${category.bathtub ne 1 }">욕조 <input type="checkbox" name="bathtub" value="1"></c:if>
			                    <c:if test="${category.dry eq 1 }">드라이기 <input type="checkbox" name="dry" value="1" checked></c:if>
			                    <c:if test="${category.dry ne 1 }">드라이기 <input type="checkbox" name="dry" value="1" ></c:if>
			                    <c:if test="${category.tv eq 1 }">TV <input type="checkbox" name="tv" value="1" checked></c:if>
			                    <c:if test="${category.tv ne 1 }">TV <input type="checkbox" name="tv" value="1"></c:if>
		                    </p>
	                    </c:forEach>
	                    <br>
	                    <h5>이용 규칙</h5>
       ...
       <div class="row session-title"><h2>주소</h2></div>
  		<div>
   		<input class="form-control form-control-lg" type="text" value="${pension.pensionAddr }" id="sample5_address" placeholder="주소를 입력해주세요." name="pensionAddr" required>
   		위도<input class="form-control form-control-lg" type="text" value="${pension.hardness }"placeholder="위도를 입력해주세요." name="hardness" required>
   		경도<input class="form-control form-control-lg" type="text" value="${pension.latitude }" placeholder="경도를 입력해주세요." name="latitude" required>
  		</div>
                	...
                	<c:forEach items="${rList }" var="room">
                     <div class="col-lg-4 col-md-6">
                         <div class="dest-col ">
                             ...
                             <input class="form-control form-control-lg" type="text" value="${room.roomName }" placeholder="객실명을 입력해주세요." name="roomName" required>
                             <div class="form-floating">
                             ...
                             <p>₩ <input type="text" placeholder="숫자만 입력해주세요." name="price" value="${room.price }" required> x 박</p>
                         </div>
                     </div>
                    </c:forEach>
                </div>
